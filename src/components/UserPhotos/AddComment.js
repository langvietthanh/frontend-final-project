...
function AddComment({ photoId }) {
  const { setUserPhotos, token, setListUser, setSnackbar } =
    ...
    try {
      const fetchData = async () => {
        const res = await fetch(
          `https://34dct4-8081.csb.app/api/comment/commentsOfPhoto/${photoId}`,
          {
            method: "POST",
            headers: {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} sx={{ ml: 2 }}>
      <TextField
        ...
        sx={{ width: 800 }}
        type="textbox"
        {...register("comment", { required: "Comment is empty" })}
        placeholder="Comment"
      />
      <br />
